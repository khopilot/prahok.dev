...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if there's an initial prompt stored
      const initialPrompt = sessionStorage.getItem('initialPrompt');
      if (initialPrompt) {
        sessionStorage.removeItem('initialPrompt');
        // Redirect to editor with the prompt
        window.location.href = `/editor?prompt=${encodeURIComponent(initialPrompt)}`;
      }
    } catch (err) {
      const error = err as Error;
      // Handle specific error messages
      let errorMessage = error.message || 'មានបញ្ហាក្នុងការចូលប្រើ';
      ...
      if (errorMessage.toLowerCase().includes('invalid') || errorMessage.toLowerCase().includes('unauthorized')) {
        errorMessage = 'អ៊ីមែល ឬពាក្យសម្ងាត់មិនត្រឹមត្រូវ';
      } else if (errorMessage.toLowerCase().includes('not found')) {
        errorMessage = 'គណនីមិនមាននៅក្នុងប្រព័ន្ធ';
    ...
    <div className="min-h-screen flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-8 bg-gray-50">
        <div className="max-w-md w-full">
          ...
          <h1 className="text-3xl font-semibold text-center mb-8">ចូលទៅកាន់គណនីរបស់អ្នក</h1>
              ...
              <div className="space-y-4">
                <div className="bg-gray-100 rounded-lg p-3">
                  <p className="text-sm text-gray-700 font-khmer">
                    "បង្កើត dashboard សម្រាប់ គ្រប់គ្រងហាងអនឡាញ ជាមួយ inventory tracking"
                  </p>
                </div>
